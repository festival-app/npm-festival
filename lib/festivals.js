...
var deleteFestivalCategories = function deleteFestivalCategories(id, callback) {
  try {
    provider.getFestivalCategories(id, {}, function (err, collection) {
      ...
      if (collection.total > 0) {
        for (var i = 0; i < collection.total; ++i) {
          ...
          if (cat) {
            provider.deleteFestivalCategory(id, cat.id, function (err, category) {
            });
          }
        }
        return callback(new Error('eee'));
      }
      else {
...
var deleteFestivalPlaces = function deleteFestivalPlaces(id, callback) {
  try {
    provider.getFestivalPlaces(id, {}, function (err, collection) {
      ...
      if (collection.total > 0) {
        for (var i = 0; i < collection.total; ++i) {
          ...
          if (place) {
            provider.deleteFestivalPlace(id, place.id, function (err, p) {
...
var deleteFestival = function deleteFestival(id, options, callback) {
  ...
  try {
    ...
    deleteFestivalCategories(id, function (errCat, resultCategories) {
      ...
      deleteFestivalPlaces(id, function (errPlace, resultPlaces) {
