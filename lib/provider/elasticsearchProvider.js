...
module.exports.provider = function provider(es) {
  ...
  var getFestivals = function getFestivals(searchFestivalsRequest, callback) {
    ...
    if (searchFestivalsRequest.limit > 0) {
      searchData.size = searchFestivalsRequest.limit;
    }

    if (searchFestivalsRequest.offset > 0) {
      searchData.from = searchFestivalsRequest.offset;
    }

    es.search(searchData, config.elasticsearch.festivals, function (error, result) {
  ...
  var getFestivalEvents = function getFestivalEvents(festivalId, searchFestivalEventsRequest, callback) {
    ...
    getFestival(festivalId, function (err, festival) {
      ...
      es.search(getElasticSearchFilters(festivalId, searchFestivalEventsRequest), config.elasticsearch.events, function (error, result) {
  ...
  var getFestivalPlaces = function getFestivalPlaces(festivalId, searchFestivalPlacesRequest, callback) {
    ...
    getFestival(festivalId, function (err, festival) {
      ...
      es.search(getElasticSearchFilters(festivalId, searchFestivalPlacesRequest), config.elasticsearch.places, function (error, result) {
  ...
  var getElasticSearchFilters = function getElasticSearchFilters(festivalId, searchParameters) {
    ...
    for (var name in searchParameters) {
      ...
      if (value && name !== 'limit' && name !== 'offset') {
        ...
        searchData.query.filtered.filter.and.push(data);
      }
    }
  ...
  var getFestivalCategories = function getFestivalCategories(festivalId, searchFestivalCategoriesRequest, callback) {
    ...
    getFestival(festivalId, function (err, festival) {
      ...
      es.search(getElasticSearchFilters(festivalId, searchFestivalCategoriesRequest), config.elasticsearch.categories, function (error, result) {
