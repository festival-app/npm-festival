    ...
    return es.search(getElasticSearchFilters(searchRequest), config.elasticsearch.festivals,
      function (error, result) {
        ...
        if (result.hits && result.hits.total > 0 && result.hits.hits) {
          return async.map(result.hits.hits, getSource, function (errHits, festivals) {
            if (errHits) {
  ...
  var getFestivalEvents = function getFestivalEvents(festivalId, searchRequest, callback) {
    ...
    return es.search(getElasticSearchFilters(searchRequest), config.elasticsearch.events,
      function (error, result) {
        ...
        if (result.hits && result.hits.total > 0 && result.hits.hits) {
          return async.map(result.hits.hits, getSource, function (errHits, events) {
            if (errHits) {
              logger.warn('Unable to get results: ', errHits);
              return callback(new ServiceUnavailableError('Unable to create festival event'));
            }
  ...
  var getFestivalPlaces = function getFestivalPlaces(festivalId, searchRequest, callback) {
    ...
    return es.search(getElasticSearchFilters(searchRequest), config.elasticsearch.places,
      function (error, result) {
        ...
        if (result.hits && result.hits.total > 0 && result.hits.hits) {
          return async.map(result.hits.hits, getSource, function (errHits, places) {
            if (errHits) {
              logger.warn('Unable to get results: ', errHits);
              return callback(new ServiceUnavailableError('Unable to create festival event'));
            }
  ...
  var getFestivalCategories = function getFestivalCategories(festivalId, searchRequest, callback) {
    ...
    return es.search(getElasticSearchFilters(searchRequest), config.elasticsearch.categories,
      function (error, result) {
        ...
        if (result.hits && result.hits.total > 0 && result.hits.hits) {
          return async.map(result.hits.hits, getSource, function (errHits, categories) {
            if (errHits) {
              logger.warn('Unable to get results: ', errHits);
              return callback(new ServiceUnavailableError('Unable to create festival event'));
            }
  ...
  var getNewsCollection = function getNewsCollection(searchRequest, callback) {
    ...
    return es.search(getElasticSearchFilters(searchRequest), config.elasticsearch.news,
      function (error, result) {
        ...
        if (result.hits && result.hits.total > 0 && result.hits.hits) {
          return async.map(result.hits.hits, getSource, function (errHits, news) {
            if (errHits) {
              logger.warn('Unable to get results: ', errHits);
              return callback(new ServiceUnavailableError('Unable to create festival event'));
            }
