...
var buildEventResponse = function buildEventResponse(event, callback) {
  getPlaceAndCategory(event.festival, event.place, event.category, function (err, result) {
    return callback(null, eventBuilders.buildEventResponse(event, result.place, result.category));
  });
};
...
var getFestivalEventsV1 = function getFestivalEventsV1(req, res, next) {
  ...
  festivals.getFestivalEvents(id, search, options, function (err, data) {
    ...
    async.map(data.events, buildEventResponse, function (err, events) {
      next.ifError(err);
