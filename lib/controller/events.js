  ...
  festivals.createFestivalEvent(id, event, {}, function (err, data) {
    ...
    async.parallel({
        place: function (callback) {
          festivals.getFestivalPlace(id, data.place, {}, callback);
        },
        category: function (callback) {
          festivals.getFestivalCategory(id, data.category, {}, callback);
        }
      },
...
var updateFestivalEventV1 = function updateFestivalEventV1(req, res, next) {
  ...
  festivals.updateFestivalEvent(id, eid, event, {}, function (err, data) {
    next.ifError(err);
    res.send(200, eventBuilders.buildFestivalEventResponse(data));
    return next();
  });
};
