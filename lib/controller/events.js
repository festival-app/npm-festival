...
var createFestivalEventV1 = function createFestivalEventV1(req, res, next) {
  ...
  festivals.createFestivalEvent(id, event, {}, function (err, data) {
    ...
    getPlaceAndCategory(id, data.place, data.category, function (err, res) {
      next.ifError(err);
      res.send(201, eventBuilders.buildFestivalEventResponse(data, res.place, res.category));
      return next();
    });
  });
};
...
var updateFestivalEventV1 = function updateFestivalEventV1(req, res, next) {
  ...
  festivals.updateFestivalEvent(id, eid, event, {}, function (err, data) {
    ...
    getPlaceAndCategory(id, data.place, data.category, function (err, res) {
      next.ifError(err);
      res.send(200, eventBuilders.buildFestivalEventResponse(data, res.place, res.category));
      return next();
    });
  });
};
