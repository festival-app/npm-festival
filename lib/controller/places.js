...
var PlacesCollectionBuilder = festivalsModel.model.placesCollection.PlacesCollectionBuilder;
  ...
  festivals.getFestivalPlaces(id, search, options, function (err, data) {
    ...
    if (data.places) {
      for (var i in data.places) {
        if (data.places.hasOwnProperty(i)) {
          var place = placeBuilders.buildFestivalPlaceResponse(data.places[i]);
